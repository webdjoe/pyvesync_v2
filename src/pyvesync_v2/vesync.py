...
SUPOUTLETS = ['wifi-switch-1.3', 'ESW15-USA', 'ESW01-EU', 'ESW03-USA'] #Orignal 7A Round outlet & rectangular 15A with nightlight
...
class VeSync(object):
    ...
    def get_active_time(self, devtype, cid, uuid=None):
        ...
        elif devtype == 'ESW01-EU' or devtype == 'ESW03-USA' and uuid is not None:
    ...
    def get_power(self, devtype, cid, uuid=None):
        ...
        if devtype == 'wifi-switch-1.3' and uuid is None:
            ...
            if devtype == 'ESW15-USA':
                response = self.get_15A_details(uuid)
            elif devtype == 'ESW01-EU' or devtype == 'ESW03-USA':
                response = self.get_10A_details(uuid)
            else:
                response = 0
            if response is not None and response:
...
class VeSyncSwitch(object):
    ...
    def turn_off(self):
        ...
        elif self.device_type == 'ESW01-EU' or self.device_type == 'ESW03-USA':
