...
def bump_version(version, bump_type):
    ...
    if bump_type == "minor":
        ...
        # Convert 0.70.0d0 to 0.70.0d20190424, fails when run on nondev release
...
def write_version(version):
    ...
    with open("src/pyvesync_v2/const.py") as fil:
        content = fil.read()
    ...
    with open("src/pyvesync_v2/const.py", "wt") as fil:
...
def main():
    ...
    current = Version(pv.const.__version__)
    ...
    subprocess.run(["git", "commit", "-am",
                   "Bumped version to {}".format(bumped)])
...
def test_bump_version():
    ...
    assert bump_version(Version("0.56.0.dev0"),
                        "dev") == Version("0.56.0.dev1")
